...
public class CSRUtils {
    ...
    public static void checkBasicCSRInfo(CSR csr) throws CSRException {
        if(csr == null) {
            ...
            throw new CSRException("Invalid common name sent in csr!");
        }
        else if(csr.getOrganizationUnit().equals("") || csr.getOrganizationUnit() == null || csr.getOrganizationUnit().length() < 3) {
            throw new CSRException("Invalid organization unit sent in csr!");
        }
        else if(csr.getOrganization().equals("") || csr.getOrganization() == null || csr.getOrganization().length() < 3) {
            throw new CSRException("Invalid organization sent in csr!");
        }
        else if(csr.getCity().equals("") || csr.getCity() == null || csr.getCity().length() < 3) {
            throw new CSRException("Invalid city sent in csr!");
        }
        else if(csr.getState().equals("") || csr.getState() == null || csr.getState().length() < 3) {
            throw new CSRException("Invalid state sent in csr!");
        }
        else if(csr.getCountry().equals("") || csr.getCountry() == null || csr.getCountry().length() < 3) {
            throw new CSRException("Invalid country sent in csr!");
        }
    }
}
