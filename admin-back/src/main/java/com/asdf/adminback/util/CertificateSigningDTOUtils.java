...
public class CertificateSigningDTOUtils {
    ...
    public static void checkBasicCertificateInfo(CertificateSigningDTO csDTO) throws CertificateSigningDTOException {
        ...
        if(commonName == null || commonName.length() < 2) {
            throw new CertificateSigningDTOException("Invalid common name field. Should have minimum 2 characters.");
        }

        String organization = csDTO.getCertificateDataDTO().getOrganization();
        if(organization == null || organization.length() < 2) {
            throw new CertificateSigningDTOException("Invalid organization field! Should have minimum 2 characters.");
        }

        String organizationUnit = csDTO.getCertificateDataDTO().getOrganizationUnit();
        if(organizationUnit == null || organizationUnit.length() < 2) {
            throw new CertificateSigningDTOException("Invalid organization unit field! Should have minimum 2 characters.");
        }

        String state = csDTO.getCertificateDataDTO().getState();
        if(state == null || state.length() < 2) {
            throw new CertificateSigningDTOException("Invalid state field! Should have minimum 2 characters.");
        }

        String country = csDTO.getCertificateDataDTO().getCountry();
        if(country == null || country.length() < 2) {
            throw new CertificateSigningDTOException("Invalid country field! Should have minimum 2 characters.");
        }
    }
